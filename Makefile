...
run-scripts:
	docker exec -i $(shell docker-compose ps -q php) /var/www/html/entrypoint.sh

mysql-init:
	@docker exec -i $(shell docker-compose ps -q mysqldb) mysql -u"$(MYSQL_ROOT_USER)" -p"$(MYSQL_ROOT_PASSWORD)" "$(MYSQL_DATABASE)" < sql/db.sql
